...
async function generateWelcomeImage({ name, group, count, avatarUrl, bgUrl }) {
  ...
  try {
    ...
    // fallback warna gradien sederhana
    const svgFallback = `
  ...
  try {
    ...
    avatarBuffer = await sharp(Buffer.from(avatarResp.data))
      .resize(180, 180)
      .composite([{ input: Buffer.from(`<svg><circle cx="90" cy="90" r="90" fill="none"/></svg>`) }])
      .png()
      .toBuffer();
  } catch {
  ...
  <svg width="${width}" height="${height}">
    ...
    <text x="250" y="120" class="group">${group}</text>
    <text x="250" y="170" class="welcome">Selamat datang,</text>
    <text x="250" y="215" class="name">${name}</text>
    <text x="250" y="260" class="count">Kamu member ke-${count}</text>
